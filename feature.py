...
class ImageFeature(object):
    def __init__(self, image, params):
        ...
        self.matcher = params.descriptor_matcher  # Matches descriptor of one image with all descriptors of second image 
    ...
    def extract(self):
        self.keypoints = self.detector.detect(self.image)  # Detected keypoints by the feature detector
        self.keypoints, self.descriptors = self.extractor.compute(  #After detecting keypoint we go on to compute a descriptor 
                self.image, self.keypoints)                         #for every one of them. 
                                                                    #Feature descriptors encode interesting information into a series of numbers
                                                                    # and act as a sort of numerical “fingerprint” that can be used to 
    ...
    def get_unmatched_keypoints(self):
        ...
        with self._lock:        # What's this lock????
