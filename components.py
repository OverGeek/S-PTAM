...
class StereoFrame(Frame):
    ...
    def triangulate(self):
        kps_left, desps_left, idx_left = self.left.get_unmatched_keypoints() # keypoints, descriptiors, index 
    ...
    # returns a list of 3D points for the map, estimated from good keypoints from the stereo images
    def triangulate_points(self, kps_left, desps_left, kps_right, desps_right):  
        matches = self.feature.row_match(
            kps_left, desps_left, kps_right, desps_right)   #take only matching keypoints from the keypoints based on
        assert len(matches) > 0                             # left and right descriptors

        px_left = np.array([kps_left[m.queryIdx].pt for m in matches])     #get left and right matched keypoints
        px_right = np.array([kps_right[m.trainIdx].pt for m in matches])

        points = cv2.triangulatePoints(
            self.left.projection_matrix, 
            self.right.projection_matrix,               # Construct 3-D points by using matched keypoints from stereo images
            px_left.transpose(), 
            px_right.transpose() 
            ).transpose()  # shape: (N, 4)

        points = points[:, :3] / points[:, 3:]        # Homogenize
